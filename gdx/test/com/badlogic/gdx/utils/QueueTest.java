...
public class QueueTest {
	...
	/** Additional tests to improve mutation coverage and edge case handling. Tests cover: - Constructor variations - Boundary
	 * conditions and exceptions - Null value handling - Iterator edge cases - Wrapped queue operations - Hash code and equals
	 * consistency */

	// ========== ADDITIONAL TESTS FOR IMPROVED MUTATION COVERAGE ==========

	/** Test constructors with different initial sizes and verify initial state. Test that queue can grow beyond initial size. */
	@Test
	...
	/** Tests the isEmpty and notEmpty methods. Verifies that: A newly created queue reports as empty A queue with elements reports
	 * as not empty A queue returns to empty state after removing all elements */
	@Test
	...
	/** Tests the ensureCapacity method to verify proper capacity management. Verifies that: The queue can be pre-allocated to
	 * avoid multiple resizes Adding elements after ensuring capacity does not trigger additional resizing All elements maintain
	 * their correct order and values after capacity increase The queue size is correctly maintained throughout capacity
	 * operations */
	@Test
	...
	/** Tests the removeValue method to verify correct removal of elements by value. Verifies that: Existing values are removed
	 * correctly Non-existing values do not affect the queue Identity comparison works as expected Removal works correctly in
	 * wrapped queues */
	@Test
	...
	/** Tests the toString method with custom separator functionality. Verifies that: The queue correctly formats its contents as a
	 * string using the provided separator Empty queues return empty strings Single elements are formatted without separators
	 * Multiple elements are properly joined with the specified separator Different separator characters work correctly Wrapped
	 * queues maintain proper order in output */
	@Test
	...
	/** Tests the equalsIdentity method for identity-based comparison. Verifies that: Empty queues are equal Same reference returns
	 * true Queues with same object instances are equal Queues with different instances (same content) are not equal Null
	 * comparison returns false Comparison with different types returns false Queues with different sizes are not equal */
	@Test
	...
	/** Tests null value handling across all queue operations. Verifies that: Null values can be added, retrieved, searched, and
	 * removed properly Queue maintains correct size with null elements indexOf works with null values using both identity and
	 * equals comparison Removal operations correctly handle null returns */
	@Test
	...
	/** Tests the QueueIterator reset method for restarting iteration. Verifies that: Reset allows re-iteration from the beginning
	 * hasNext returns correct state after reset All elements are accessible again in proper order after reset */
	@Test
	...
	/** Tests the indexOf method when values are not found in the queue. Verifies that: indexOf returns -1 for non-existing values
	 * with identity comparison indexOf returns -1 for non-existing values with equals comparison indexOf returns -1 when searching
	 * empty queues */
	@Test
	...
	/** Tests queue operations when the internal array is wrapped (head > tail). Verifies that: get, first, and last methods work
	 * correctly in wrapped state removeIndex works correctly when elements wrap around the backing array Element order and values
	 * are maintained properly in wrapped state */
	@Test
	...
	/** Tests the clear method on empty and populated queues. Verifies that: Clearing empty queues does not throw exceptions
	 * Populated queues are properly emptied Multiple consecutive clear operations are handled safely */
	@Test
	...
	/** Tests hash code consistency and equality contract for queues. Verifies that: Equal queues produce identical hash codes Hash
	 * codes remain consistent across multiple calls Empty queues have matching hash codes Null values are properly handled in hash
	 * code calculation */
	@Test
	...
	/** Tests the equals method with various object types and null values. Verifies that: equals returns false for null objects
	 * equals returns false for non-Queue objects equals returns true for same reference Queues containing null values are
	 * correctly compared Queues with null and non-null elements at same position are properly distinguished */
	@Test
	...
	/** Tests the ensureCapacity method at boundary conditions. Verifies that: ensureCapacity with zero additional capacity does
	 * not resize Requesting capacity for additional elements properly resizes the backing array Elements can be added after
	 * capacity increase without further resizing All values remain correct and accessible after capacity operations */
	@Test
	...
	/** Tests the indexOf method at various boundary positions in the queue. Verifies that: indexOf correctly locates elements at
	 * head position indexOf correctly locates elements at tail-1 position indexOf correctly locates elements at middle position
	 * indexOf works correctly in wrapped queues where elements span across the array boundary Index calculation is correct for
	 * both continuous and wrapped queue configurations */
	@Test
	...
	/** Tests the removeIndex method's return value functionality. Verifies that: removeIndex correctly returns the removed element
	 * Queue maintains proper size after removal Element order is preserved after removal in continuous arrays Removal works
	 * correctly in wrapped queue configurations where elements span the array boundary */
	@Test
	...
	/** Tests the clear method on wrapped queues where elements span array boundaries. Verifies that: Clearing a wrapped queue
	 * (head > tail) properly nullifies all elements in both array segments Head and tail indices are reset to zero after clearing
	 * Size is set to zero after clearing Queue is left in a valid empty state that can accept new elements without issues */
	@Test
	...
	/** Tests the iterator behavior in non-allocating mode where iterator instances are reused. Verifies that:
	 * Collections.allocateIterators=false enables iterator pooling/reuse Iterators can be used sequentially without interference
	 * when pooling is enabled Each iterator properly iterates through all elements from the beginning Original allocateIterators
	 * setting is restored via finally block after test completion */
	@Test
	...
	/** Tests the hashCode method with different values and configurations. Verifies that: Queues with different content produce
	 * different hash codes Queues with null values produce different hash codes than non-null values Hash code calculation is
	 * consistent regardless of wrapped state (head > tail) or continuous state (head < tail) Logically equivalent queues with same
	 * values produce identical hash codes even when internal array layout differs */
	@Test
	...
	/** Tests the automatic resizing behavior of the queue at capacity boundaries. Verifies that: Adding elements beyond initial
	 * capacity triggers automatic resizing All elements maintain correct values and order after resize Resizing works correctly
	 * when elements are added sequentially via addLast Resizing properly handles wrapped queues (head > tail) where elements span
	 * the array boundary Queue remains in a valid state with proper element ordering after resize */
	@Test
